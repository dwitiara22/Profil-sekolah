...
class HatcheryController extends Controller
{
   ...
   public function update(Request $request, TimKerja $hatchery)
    {
        // 1. Validasi Input
        $request->validate([
            'nama_lengkap' => 'required|string|max:255',
        ...
        if ($request->hasFile('foto')) {
            ...
            if ($hatchery->foto) {
                Storage::disk('public')->delete($hatchery->foto);
            }
    ...
    public function destroy(TimKerja $hatchery)
    {
        if($hatchery->foto) {
            Storage::disk('public')->delete($hatchery->foto);
        }
