...
class HumasController extends Controller
{
  public function index()
   ...
   public function update(Request $request, TimKerja $humas)
    {
        // 1. Validasi Input
        $request->validate([
            'nama_lengkap' => 'required|string|max:255',
        ...
        if ($request->hasFile('foto')) {
            ...
            if ($humas->foto) {
                Storage::disk('public')->delete($humas->foto);
            }
    ...
    public function destroy(TimKerja $humas)
    {
        if($humas->foto) {
            Storage::disk('public')->delete($humas->foto);
        }
