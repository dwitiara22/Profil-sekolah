...
class KesiswaanController extends Controller
{
   ...
   public function update(Request $request, TimKerja $kesiswaan)
    {
        // 1. Validasi Input
        $request->validate([
            'nama_lengkap' => 'required|string|max:255',
        ...
        if ($request->hasFile('foto')) {
            ...
            if ($kesiswaan->foto) {
                Storage::disk('public')->delete($kesiswaan->foto);
            }
    ...
    public function destroy(TimKerja $kesiswaan)
    {
        if($kesiswaan->foto) {
            Storage::disk('public')->delete($kesiswaan->foto);
        }
