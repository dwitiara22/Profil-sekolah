...
class PengolahanController extends Controller
{
   ...
   public function update(Request $request, TimKerja $pengolahan)
    {
        // 1. Validasi Input
        $request->validate([
            'nama_lengkap' => 'required|string|max:255',
        ...
        if ($request->hasFile('foto')) {
            ...
            if ($pengolahan->foto) {
                Storage::disk('public')->delete($pengolahan->foto);
            }
    ...
    public function destroy(TimKerja $pengolahan)
    {
        if($pengolahan->foto) {
            Storage::disk('public')->delete($pengolahan->foto);
        }
