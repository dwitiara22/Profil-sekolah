...
class RtpController extends Controller
{
   ...
   public function update(Request $request, TimKerja $rtp)
    {
        // 1. Validasi Input
        $request->validate([
            'nama_lengkap' => 'required|string|max:255',
        ...
        if ($request->hasFile('foto')) {
            ...
            if ($rtp->foto) {
                Storage::disk('public')->delete($rtp->foto);
            }
    ...
    public function destroy(TimKerja $rtp)
    {
        if($rtp->foto) {
            Storage::disk('public')->delete($rtp->foto);
        }
