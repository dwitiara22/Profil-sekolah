...
class TambakController extends Controller
{
   ...
   public function update(Request $request, TimKerja $tambak)
    {
        // 1. Validasi Input
        $request->validate([
            'nama_lengkap' => 'required|string|max:255',
    ...
    public function destroy(TimKerja $tambak)
    {
        if($tambak->foto) {
            Storage::disk('public')->delete($tambak->foto);
        }
